...
use crate::types::minecraft_version::MinecraftVersion;
...
// pub async fn download_latest_server(folder: &String, settings: &mut Settings)
// -> Result<()> {   let mut headers = HeaderMap::new();
//   headers.insert(
//     "Accept",
//     HeaderValue::from_static("text/html,application/xhtml+xml,application/
// xml;q=0.9,image/avif,image/webp,*/*;q=0.8"),   );
//   headers.insert("Accept-Language",
// HeaderValue::from_static("en-GB,en;q=0.5"));   headers.insert("
// Accept-Encoding", HeaderValue::from_static("identity"));   headers.insert("
// DNT", HeaderValue::from_static("1"));   headers.insert("Connection",
// HeaderValue::from_static("keep-alive"));   headers.insert("
// Upgrade-Insecure-Requests", HeaderValue::from_static("1"));
...
//   let server_webpage =
// client.get(SERVER_WEBPAGE_URL).send().await?.text().await?;

//   let server_info = get_server_info(&server_webpage)?;

//   if !settings::is_latest_minecraft_version_with_update(settings,
// server_info.version) {     let mut server_file =
// File::create(Path::new(folder).join(SERVER_FILENAME))?;     std::io::copy(
//       &mut
// Cursor::new(client.get(server_info.download_url).send().await?.bytes().await?
// ),       &mut server_file,
//     )?;
//   }
